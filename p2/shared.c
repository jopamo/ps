...
#if SHM_DEBUG
  char log_msg[256];
  snprintf( log_msg, sizeof( log_msg ), "Shared memory segment created/accessed. SHMID: %d, Size: %zu bytes", shmid,
            size );
  debug_log( log_msg );
#endif
...
#if SHM_DEBUG
  char log_msg[256];
  snprintf( log_msg, sizeof( log_msg ), "Attached shared memory (read/write). SHMID: %d, Address: %p", shmid, addr );
  debug_log( log_msg );
#endif
...
#if SHM_DEBUG
  char log_msg[256];
  snprintf( log_msg, sizeof( log_msg ), "Attached shared memory (read-only). SHMID: %d, Address: %p", shmid, addr );
  debug_log( log_msg );
#endif
